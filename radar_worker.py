...
class BringATrailerAdapter(JSONLDListingAdapter):
    ...
    INDEX = "https://bringatrailer.com/auctions/"
    # Confirm this pattern against the live auctions index; listing URLs look
    # like https://bringatrailer.com/listing/<slug>/
    LISTING_RE = re.compile(r"https://bringatrailer\.com/listing/[a-z0-9][a-z0-9\-]+/?", re.I)

    def discover(self, fetcher: Fetcher) -> Iterator[str]:
        resp = fetcher.get(self.INDEX)
        if not resp:
            return
        for href in sorted(set(self.LISTING_RE.findall(resp.text))):
